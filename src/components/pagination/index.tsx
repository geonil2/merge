...
export const DEFAULT_LISTS_COUNT = 20;

const Pagination: React.FC<Props> = ({ totalCount }) => {
  const [offset, setOffset] = useRecoilState(offsetAtom);
  const initialPage = offset / DEFAULT_LISTS_COUNT;

  const getTotalCount = () => {
    return totalCount && totalCount > DEFAULT_LISTS_COUNT
      ? Math.floor(totalCount / DEFAULT_LISTS_COUNT) + 1
      : 1;
  };
    ...
    <S.PaginationWrap>
      <ReactPaginate
        ...
        onPageChange={(selectedItem) =>
          setOffset(selectedItem.selected * DEFAULT_LISTS_COUNT)
        }
        pageRangeDisplayed={4} // 표시되는 페이지 범위
        marginPagesDisplayed={1} // 표시되는 페이지 범위
        previousLabel={arrowLeft}
        pageCount={getTotalCount()} // 총 페이지 수
