import React, { useCallback } from "react";
import { useRouter } from "next/router";
import ReactTimeago from "react-timeago";
import { Interweave } from "interweave";
import { debounce } from "lodash";

import * as S from "./style";

import { useMutation, useQueryClient } from "@tanstack/react-query";

import { useSetRecoilState } from "recoil";
import {
  basicPopupContentsAtom,
  visibleModalAtom,
} from "../../../recoil/modal";

import { Board, BoardByIdQueryKey } from "../../../services/board/types";
import { postLikeApi } from "../../../services/like/api";
import { deleteBoardByIdApi } from "../../../services/board/api";

import { popupModalContents } from "../../../resources/types";
import UpdateDeleteButtonWrapper from "../../updateDeleteButtonWrapper";
import HeadMeta from "../../headMeta";
import { getImageByDescription } from "../../listThumbnail";

interface Props {
  contents: Board;
  userId?: string;
}

const BoardContents: React.FC<Props> = ({ contents, userId }) => {
  const updateLike = useMutation(postLikeApi);
  const deleteBoard = useMutation(deleteBoardByIdApi);
  const queryClient = useQueryClient();
  const setBasicPopupContents = useSetRecoilState(basicPopupContentsAtom);
  const setVisibleModal = useSetRecoilState(visibleModalAtom);
  const router = useRouter();

  const onClickLike = useCallback(
    debounce(() => {
      updateLike.mutate(contents._id, {
        onSuccess: () => {
          queryClient.invalidateQueries([
            BoardByIdQueryKey,
            { boardId: contents._id },
          ]);
        },
      });
    }, 200),
    [updateLike.data]
  );

  const removeBoard = () => {
    deleteBoard.mutate(contents._id, {
      onSuccess: () => {
        router.push(`/${contents.category}`);
      },
    });
  };

  const onClickDeleteButton = () => {
    setVisibleModal(true);
    setBasicPopupContents({
      ...popupModalContents.deleteBoard,
      onClick: removeBoard,
    });
  };

  const onClickUpdateButton = () => {
    router.push(
      {
        pathname: `/writing/${contents._id}`,
        query: {
          id: contents._id,
          category: contents.category,
          title: contents.title,
          description: contents.description,
        },
      },
      `/writing/${contents._id}`
    );
  };

  return (
    <>
      <HeadMeta
        title={contents.title}
        description={contents.title}
        url={`${process.env.NEXT_PUBLIC_URL}/${contents._id}`}
        image={getImageByDescription(contents.description)}
      />
      <S.Container>
        <p>Question</p>
        <S.ContentsHeader>
          <S.Owner>
            <S.Thumbnail src="/images/icons/profile.svg" />
            <S.UserText>
              <p>{contents.owner?.name}</p>
              <S.Email>{contents.owner?.email}</S.Email>
            </S.UserText>
          </S.Owner>
          <S.HeaderRight>
            <S.LikesCount>좋아요 {contents.likes?.length}</S.LikesCount>
            <UpdateDeleteButtonWrapper
              owner={contents.owner?._id}
              onClickUpdateButton={onClickUpdateButton}
              onClickDeleteButton={onClickDeleteButton}
            />
          </S.HeaderRight>
        </S.ContentsHeader>
        <S.ContentsBody>
          <S.TitleWrap>
            <S.Title>{contents.title}</S.Title>
            <S.Date>
              <ReactTimeago date={contents.createdAt} />
            </S.Date>
          </S.TitleWrap>
          <S.Description>
            <Interweave content={contents.description} />
          </S.Description>
          {userId && (
            <S.Likes active={contents.likes?.includes(userId) ? true : false}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 512 512"
                onClick={() => onClickLike()}
              >
                <path d="M313.4 32.9c26 5.2 42.9 30.5 37.7 56.5l-2.3 11.4c-5.3 26.7-15.1 52.1-28.8 75.2H464c26.5 0 48 21.5 48 48c0 25.3-19.5 46-44.3 47.9c7.7 8.5 12.3 19.8 12.3 32.1c0 23.4-16.8 42.9-38.9 47.1c4.4 7.2 6.9 15.8 6.9 24.9c0 21.3-13.9 39.4-33.1 45.6c.7 3.3 1.1 6.8 1.1 10.4c0 26.5-21.5 48-48 48H294.5c-19 0-37.5-5.6-53.3-16.1l-38.5-25.7C176 420.4 160 390.4 160 358.3V320 272 247.1c0-29.2 13.3-56.7 36-75l7.4-5.9c26.5-21.2 44.6-51 51.2-84.2l2.3-11.4c5.2-26 30.5-42.9 56.5-37.7zM32 192H96c17.7 0 32 14.3 32 32V448c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V224c0-17.7 14.3-32 32-32z" />
              </svg>
              <div>좋아요 {contents.likes?.length}</div>
            </S.Likes>
          )}
        </S.ContentsBody>
      </S.Container>
    </>
  );
};

export default BoardContents;
