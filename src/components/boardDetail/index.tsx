...
const BoardDetail: React.FC<Prop> = ({ boardId }) => {
  ...
  const { data: contents } = useQuery([BoardByIdQueryKey, { boardId }], () =>
    getBoardByIdApi(boardId)
  );
  const { data: comment } = useQuery(
    [CommentByBoardIdQueryKey, { boardId }],
    () => getCommentByBoardIdApi(boardId)
  );

  return (
    <>
      {contents && <BoardContents contents={contents} userId={user?._id} />}
      {user && (
        <CommentWrite userId={user._id} boardId={boardId} name={user.name} />
      )}
      {comment && comment.length !== 0 && (
        <CommentListContainer userId={user?._id} comments={comment} />
      )}
    </>
  );
};
