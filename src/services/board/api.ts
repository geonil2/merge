...
import {
  ...
  BoardByCategoryResponseData,
  BoardByIdResponseData,
  BoardResponseData,
...
export const getBoardByCategoryApi = async ({
  category,
  offset,
  limit,
}: BoardByCategoryRequestQuery) => {
  const { data } = await API.get<BoardByCategoryResponseData>(`/api/boards`, {
...
export const updateBoardByIdApi = async (boardInfo: PutBoardRequestBody) => {
  ...
  const { data } = await API.put(`/api/boards/${boardId}`, {
    title,
    description,
    category,
    email,
